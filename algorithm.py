...
def stoploss_hit(timestamp):
    ...
    for index in range(length):
        ...
        if trade.buy_or_sell == "sell" and trade.type == 2 and trade.check_stoploss_hit(options, timestamp, stoploss[trade.ins_type]):
...
def type2_purchase(timestamp):
    ...
    for trade in open_postions:
        if has_type_2(open_postions) == False and trade.buy_or_sell == "sell" and trade.premium_more_than(options, timestamp, 45) == True:
...
def close_type2(timestamp):
    ...
    for index in range(length):
        ...
        if trade.buy_or_sell == "sell" and trade.type == 2 and trade.premium_less_than(options, timestamp, 20):
            ...
            try:
                ...
                print("[ERROR] Unable to close, {}, {}, {}, {}".format(trade.ins_type, trade.strike_price, trade.expiry, trade.expiry))
    # for type in types:
    #     open_trade(type, 2, "Profit Premimum below 20, bying again", timestamp)            
    for index in reversed(to_pop):
        open_postions.pop(index)
...
def stoploss_nifty(timestamp):
    ...
    if hour["rsi"] > 65:
        ...
        if nifty_current < hour["supertrend"] <= nifty_prev:
            close_all("PE", "RSI > 65 and nifty crossunder supertrend", timestamp)

    if hour["rsi"] < 35:

        if nifty_prev <= hour["supertrend"] < nifty_current:
            close_all("CE", "RSI < 35 and nifty crossover supertrend", timestamp)
...
for day in days:
    ...
    for hour in hours:
        ...
        if len(options) != 0:
            ...
            if day["rsi"] < 40:
                ...
                if crossover(hour["rsi"], prev_hour["rsi"], 70):
                    close_all("CE", "40 < RSI-D < 60, crossover 65",
                              hour["timestamp"])
                if crossunder(hour["rsi"], prev_hour["rsi"], 30):
                    close_all("PE", "40 < RSI-D < 60, crossunder 35",
                              hour["timestamp"])

            # if hour["rsi"] < 50:
            #     open_trade("CE", 1, "RSI-D < 40, RSI-H < 50",
            #                hour["timestamp"])

            if hour["rsi"] < 35:
                close_all("PE", "RSI-D < 40, RSI-H < 35", hour["timestamp"])

            # if crossunder(hour["rsi"], prev_hour["rsi"], 35):
            ...
            if not has_sell(open_postions):
                if prev_hour["direction"] == 1 and hour["direction"] == -1:
                    open_trade(
                        "CE", 1, "RSI-D < 40, no open positions and supertrend changes from down to up", hour["timestamp"])

        if 40 < day["rsi"] < 60:

            # if hour["rsi"] < 40:
            #     open_trade("CE", 1, "40 < RSI-D < 60, RSI-H < 40",
            #                hour["timestamp"])

            # if 40 < hour["rsi"] < 60:
            #     open_trade(
            #         None, 1, "40 < RSI-D < 60, 40 < RSI-H < 60", hour["timestamp"])

            # if 60 < hour["rsi"]:
            #     open_trade("PE", 1, "40 < RSI-D < 60, 60 < RSI-H",
            #                hour["timestamp"])
            ...
            if crossover(hour["rsi"], prev_hour["rsi"], 45):
                open_trade("PE", 1, "60 < RSI-D< 60, crossover 45",
                           hour["timestamp"])     
            if crossover(hour["rsi"], prev_hour["rsi"], 50):
                open_trade("PE", 1, "60 < RSI-D< 60, crossover 50",
                           hour["timestamp"])   
            if crossover(hour["rsi"], prev_hour["rsi"], 55):
                open_trade("PE", 1, "60 < RSI-D< 60, crossover 55",
                           hour["timestamp"])              

            if crossunder(hour["rsi"], prev_hour["rsi"], 55):
                open_trade("CE", 1, "45 < RSI-D < 60, crossunder 55",
                            hour["timestamp"])
            if crossunder(hour["rsi"], prev_hour["rsi"], 50):
                open_trade("CE", 1, "45 < RSI-D < 60, crossunder 50",
                            hour["timestamp"])
            if crossunder(hour["rsi"], prev_hour["rsi"], 45):
                open_trade("CE", 1, "45 < RSI-D < 60, crossunder 45",
                            hour["timestamp"])
        ...
        if 60 < day["rsi"]:
            ...
            # if 50 < hour["rsi"]:
            #     open_trade("PE", 1, "60 < RSI-D, 50 < RSI-H",
            #                hour["timestamp"])

            # if 65 < hour["rsi"]:
            #     # uptrend in supertrend
            #     if hour["direction"] == 1:
            #         stoploss["PE"] = hour["supertrend"]
            #         stoploss["CE"] = hour["supertrend"]
            ...
            #                hour["timestamp"])
            if crossover(hour["rsi"], prev_hour["rsi"], 60):
                open_trade("PE", 1, "60 < RSI-D, crossover 60",
                           hour["timestamp"])
            ...
            if day["is_week_start"] == True and timestamp.time() == time(hour=15, minute=30):
                ...
                for trade in open_postions:
                    ...
                    try:
                        ...
                        if action == "buy":
                            closed_types.append(trade.type)
